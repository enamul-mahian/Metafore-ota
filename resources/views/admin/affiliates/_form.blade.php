@if($errors->any())<div class="egh-alert" style="background:#fee2e2;color:#991b1b"><ul style="margin:0;padding-left:20px">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
...
</div><div style="display:flex;gap:10px;margin-top:18px"><button class="egh-button">{{ $submitLabel }}</button><a class="egh-button secondary" href="{{ route('admin.affiliates.index') }}">Cancel</a></div>
